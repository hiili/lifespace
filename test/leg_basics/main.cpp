...
static Controller::ControlMap * newCameraKeymap()
{
  ...
  // define the mouse controls. the last argument controls sensitivity.
  keymap[GLOWViewport::MOUSE_DX] =
    Controller::MapTarget( FloatingActor::CTRL_TORQUEREL_Y_NEG, 2.0 );
  keymap[GLOWViewport::MOUSE_DY] =
    Controller::MapTarget( FloatingActor::CTRL_TORQUEREL_X_NEG, 2.0 );
...
static Controller::ControlMap * newLegKeymap()
{
  Controller::ControlMap * keymap_ptr = new Controller::ControlMap();
  Controller::ControlMap & keymap = *keymap_ptr;
  ...
  keymap['h'] = Controller::MapTarget(Leg::CTRL_LOWERPART_TORQUE, -0.5);
  
  keymap['u'] = Controller::MapTarget(Leg::CTRL_ENDEFFECTOR_TORQUE_X, 0.2);
  keymap['j'] = Controller::MapTarget(Leg::CTRL_ENDEFFECTOR_TORQUE_X, -0.2);
...
public:
  ...
  Base()
  {
    ...
    shared_ptr<Shape> shape
      ( shapes::Cube::create( makeVector3d( 0.8, 0.2, 0.8 ) ));
    shared_ptr<BasicLocator> locator( new BasicLocator() );
    setLocator( locator );
...
int main( int argc, char * argv[] )
{
  ...
  viewport.addActor( leg, newLegKeymap() );
