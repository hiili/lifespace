...
int main( int argc, char * argv[] )
{
  ...
  deserializer.addTargetObject( cameraObject, WorldSerialization::PROP_ALL );
  deserializer.addTargetObject( jointObject1, WorldSerialization::PROP_ALL );
  deserializer.addTargetObject( tube, WorldSerialization::PROP_ALL );
  deserializer.addTargetObject( cube, WorldSerialization::PROP_ALL );
  //deserializer.addTargetObject( ballSpace, WorldSerialization::PROP_ALL, true );
  deserializer.addSourceStream( &cin );
  ...
  window.events.addListener( &deserializer );
  window.events.addListener( &viewport );   // refresh will probably happen
                                            // lastly anyways, independent of
                                            // this order
  GLOWDevice::MainLoop();
